...
export const getResource = async function(lessonId, resourceId, userName){
  try{
    const filter = { "lessons._id": lessonId };
    ...
    if (lesIndex !== -1 && userName === thisCourse.courseCreator) {
      ...
      return thisCourse.lessons[lesIndex].lessonFilesNLinks[resIndex] 
    } else {
      return false;
    }
  } catch (err) {
...
export const deleteResource = async function (lessonId, resId, userName) {
  ...
  if (lesIndex !== -1 && userName === doc.courseCreator) {
