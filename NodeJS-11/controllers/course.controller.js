...
export const getCourse = async function (req, res, next) {
  try {
...
export const deleteCourse = async function (req, res, next) {
  ...
  try {
    const cid = req.params.courseId;
    const doc = await CourseService.findCourse(cid).catch((err) => {
      throw err;
    });
    if (!doc){
      throw new AppErrors.Error400("There is no course with such id");
    }
    if (AccessController.userIsCreator(doc, req.user.username)) {
      ...
      throw new AppErrors.Error403("Only the creator has permition to delete");
    }
  } catch (error) {
    next(error);
  }
